...
`GET /api/tags` returns tag names and note/task counts, counting rows that inherit a tag from a section row or an ancestor. `GET /api/journal?tag=work` returns matching branches, retaining original parent IDs; filtered roots may have parents omitted from the response. Every journal entry carries `inherited_tags`, the tags it holds through its section row or its ancestors without storing them itself. The current date remains available for new entries. There is no separate tag registry or stale tag counter.
...
| `entries` | Note/task kind, optional day, Markdown content, JSON tags, role (plain or wait), parent entry, shared sibling position, timestamps, retry ID |
