...
`q` is a case-insensitive literal substring of Markdown, rendered text, or extracted link URLs. `tag` matches a normalized tag name, held by the bullet itself or inherited (see `inherited_tags`). Together they use AND. Matches include all descendants and enough ancestor context to preserve the tree; `matched` distinguishes direct matches from context. An unrelated sibling is not included. Filters apply only to bullets and tasks: focus time is not attributed to tags or text, so focus totals remain unfiltered.

`tasks=visible` (default) returns the current to-do tree, including completed children still retained under an open parent. `tasks=all` also includes completed task trees. `tasks=none` omits the current to-do snapshot. Fully completed root task trees appear in their completion day’s logbook bullets, interleaved with notes, while retaining their task kind and checked state. `next_url` sets `tasks=none` so a traversal does not repeat the current to-do snapshot.
...
- A root note whose `content_markdown` starts with `# ` is a section row: a heading that groups the root bullets after it on that date until the next section row. `inherited_tags` lists the tags a bullet carries without holding them: its section row's tags, and its ancestors' tags. Inheritance follows row order and is never stored; treat `tags` plus `inherited_tags` as the bullet's effective tags.
- `role` is `""` for a plain bullet, `"scratch"` for a note the interface keeps folded away, or `"wait"` for something the parent to-do waits on; a waiting row is settled when its `completed_at` is set, and it never counts as a step of its parent.
...
- `source_task_id` is a nullable schema-v1 compatibility field. New unified entries always return null; completed tasks remain task records in the logbook.
