...
- `role` is `""` for a plain bullet or `"wait"` for something the parent to-do waits on; a waiting row is settled when its `completed_at` is set, and it never counts as a step of its parent.
